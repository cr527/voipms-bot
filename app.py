...
def sms_reply():
    ...
    if message_lower == "sexy":
        ...
        # Dummy comment to force a Render redeploy - please ignore - Fix 3 (openhue flags)
        hue_lights = [
            {"name": "TV Room Lamp", "brightness": 100, "rgb": "#FF0000"}, # Red
            {"name": "Chris\' nightstand", "brightness": 100, "rgb": "#FF0000"}, # Red
            {"name": "Jana\'s nightstand", "brightness": 100, "rgb": "#FF0000"} # Red
        ]

        for light in hue_lights:
            # Construct the OpenClaw command for Hue lights with new flags
            command = f"openhue set light \"{light[\"name\"]}\" --on " \
                      f"--brightness {light[\"brightness\"]} --rgb {light[\"rgb\"]}"
